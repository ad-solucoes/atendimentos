    ...
    <div class="bg-white border border-gray-200 rounded-xl p-4 shadow-sm mb-4">
        <div class="grid grid-cols-1 md:grid-cols-6 gap-4">
            ...
            <div class="flex flex-col">
                ...
                <input id="filtroNumeroAtendimento" type="text"
                    wire:model.live.debounce.500ms="filtroNumeroAtendimento" placeholder="Ex: A123456"
                    class="border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:outline-none">
            </div>
        ...
        <table class="hidden md:table min-w-full">
            <thead>
                <tr class="bg-gray-100 text-left text-sm text-gray-600">
                    <th class="px-2 py-2 text-center font-semibold" width="120"
                        wire:click="sortByField('atendimento_status')" style="cursor: pointer;"
                        title="Clique para ordenar">
                        Status
                        @include('livewire.partials._sort-icon', ['field' => 'atendimento_status'])
                    </th>
                    <th class="px-2 py-2 font-semibold" width="120" wire:click="sortByField('atendimento_data')"
                        style="cursor: pointer;" title="Clique para ordenar">
                        Data
                        @include('livewire.partials._sort-icon', ['field' => 'atendimento_data'])
                    </th>
                    <th class="px-2 py-2 font-semibold" width="120" wire:click="sortByField('atendimento_numero')"
                        style="cursor: pointer;" title="Clique para ordenar">
                        Número
            ...
            <tbody>
                ...
                <tr wire:loading.class.remove="hidden"
                    wire:target="filtroDataInicial, filtroDataFinal, filtroNumeroAtendimento, filtroPrioridade, filtroNome, filtroCpf, filtroSus, filtroMae"
                    class="hidden">
                    ...
                    <tr class="border-t hover:bg-gray-50 transition" wire:loading.remove
                        ...
                        <td class="px-2 py-1 text-sm font-medium text-center">
                            @if ($atendimento->solicitacoes)
                                <span
                                    class="inline-flex items-center px-3 py-0.5 rounded-full text-xs font-semibold bg-green-100 text-green-800">Concluído</span>
                            @else
                                <span
                                    class="inline-flex items-center px-3 py-0.5 rounded-full text-xs font-semibold bg-red-100 text-red-800">Aberto</span>
                            @endif
                        </td>
                        <td class="px-2 py-1 text-sm text-gray-800">
                            {{ $atendimento->atendimento_data->format('d/m/Y') }}</td>
                        <td class="px-2 py-1 text-sm font-medium text-gray-800">{{ $atendimento->atendimento_numero }}
                        </td>
                        ...
                        <td class="px-2 py-1 text-sm text-gray-800">
                            @if ($atendimento->atendimento_prioridade == 'Baixa')
                                <span
                                    class="inline-flex items-center px-3 py-0.5 rounded-full text-xs font-semibold bg-green-100 text-green-800">Baixa</span>
                            @elseif ($atendimento->atendimento_prioridade == 'Média')
                                <span
                                    class="inline-flex items-center px-3 py-0.5 rounded-full text-xs font-semibold bg-yellow-100 text-yellow-800">Média</span>
                            @elseif ($atendimento->atendimento_prioridade == 'Alta')
                                <span
                                    class="inline-flex items-center px-3 py-0.5 rounded-full text-xs font-semibold bg-red-100 text-red-800">Alta</span>
                            @endif
                        </td>
                        <td class="px-2 py-1 text-center space-x-0">
                            <!-- Exibir -->
                            @can('Exibir Atendimento')
                            ...
                            <!-- Deletar -->
                            @can('Deletar Atendimento')
                            <button type="button" wire:click="confirmDelete('{{ $atendimento->atendimento_id }}')"
