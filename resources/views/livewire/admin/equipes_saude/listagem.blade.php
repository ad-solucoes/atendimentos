<div class="space-y-6">
            ...
            <div class="flex flex-col md:items-end">
                ...
                <div class="flex items-center justify-between md:justify-end gap-3">
                    ...
                    <small class="text-gray-600 text-sm">
                        ...
                        Total: <span class="font-semibold text-gray-800">{{ $equipes_saude->total() }}</span> registro(s)
                    </small>

                    <a href="{{ route('admin.equipes_saude.formulario') }}"
                    class="inline-flex items-center justify-center gap-2 bg-blue-700 hover:bg-blue-800 text-white px-4 py-2 rounded-lg shadow transition-all duration-200 text-sm">
                        <i class="fa fa-plus"></i> Nova Equipe de Saúde
                    </a>
                </div>
            </div>
        </div>
        ...
        <table class="hidden md:table min-w-full">
            <thead>
                <tr class="bg-gray-100 text-left text-sm text-gray-600">
                    <th class="px-2 py-2 text-center font-semibold" width="100" wire:click="sortByField('equipe_saude_status')" style="cursor: pointer;" title="Clique para ordenar">
                        Status
                        @include('livewire.partials._sort-icon', ['field' => 'equipe_saude_status'])
                    </th>
                    <th class="px-2 py-2 font-semibold" wire:click="sortByField('equipe_saude_nome')" style="cursor: pointer;" title="Clique para ordenar">
                        Nome da Equipe de Saúde
                        @include('livewire.partials._sort-icon', ['field' => 'equipe_saude_nome'])
                    </th>
            ...
            <tbody>
                ...
                @forelse ($equipes_saude as $equipe_saude)
                    <tr class="border-t hover:bg-gray-50 transition" wire:loading.remove wire:target="searchTerm">
                        <td class="px-2 py-1 text-sm font-medium text-center">
                            @if($equipe_saude->equipe_saude_status)
                                ...
                                <div>
                                    @if($searchTerm)
                                        <h4 class="text-gray-700 font-semibold text-lg">
                                            Nenhum equipe de saúde encontrado
                                        </h4>
                                    ...
                                    @else
                                        <h4 class="text-gray-700 font-semibold text-lg">
                                            Nenhum equipe de saúde cadastrado
                                        </h4>
                                        <p class="text-sm text-gray-500 mt-1">
                                            Comece criando um novo equipe de saúde.
                                        </p>
                                    @endif
                                </div>

                                @if(!$searchTerm)
                                    <a href="{{ route('admin.equipes_saude.formulario') }}" class="mt-3 inline-flex items-center gap-2 bg-blue-700 hover:bg-blue-800 text-white text-sm font-medium px-4 py-2.5 rounded-lg shadow transition-all duration-200">
                                        <i class="fa fa-plus"></i> Criar Primeira Equipe de Saúde
                                    </a>
                                @endif
        ...
        <div class="block md:hidden divide-y divide-gray-100">
            @forelse ($equipes_saude as $equipe_saude)
                <div class="p-4 hover:bg-gray-50 transition">
                    <p class="font-semibold text-gray-800 mb-1">{{ $equipe_saude->equipe_saude_nome }}</p>
                    <p class="text-sm text-gray-600">
                        <strong>Status:</strong> {{ $equipe_saude->equipe_saude_status ?? '—' }}<br>
                    </p>

                    <!-- Botões -->
                    <div class="flex justify-end mt-2 gap-1">
                        <a href="/equipes_saude/{{ $equipe_saude->equipe_saude_id }}" 
            ...
            @empty
                <p class="text-center text-gray-500 py-4">Nenhum equipe_saude encontrado.</p>
            @endforelse
        </div>
    </div>
    ...
    <div class="flex flex-col md:flex-row justify-between items-center gap-3">
        ...
        @if ($equipes_saude->total() > 0)
            <div class="text-sm text-gray-600">
                Mostrando
                <span class="font-semibold text-gray-800">{{ $equipes_saude->firstItem() }}</span>
                até
                <span class="font-semibold text-gray-800">{{ $equipes_saude->lastItem() }}</span>
                de
                <span class="font-semibold text-gray-800">{{ $equipes_saude->total() }}</span>
                registro(s)
                @if ($searchTerm)
    ...
    @if($confirmingDelete)
        <div class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 px-4" style="margin-top: 0!important">
            <div class="bg-white rounded-lg p-4 w-full max-w-md shadow-lg">
                ...
                <p class="mb-4 text-gray-700">Tem certeza de que deseja excluir este equipe_saude?</p>
