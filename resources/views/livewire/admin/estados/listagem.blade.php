            ...
            <div class="flex flex-col md:items-end">
                ...
                <div class="flex items-center justify-between md:justify-end gap-3">
                    ...
                    <small class="text-gray-600 text-sm">
                        ...
                        Total: <span class="font-semibold text-gray-800">{{ $estados->total() }}</span>
                        registro(s)
                    </small>

                    @can('Adicionar Estado')
                        <a href="{{ route('admin.estados.formulario') }}"
            ...
            <thead>
                <tr class="bg-gray-100 text-left text-sm text-gray-600">
                    <th class="px-2 py-2 text-center font-semibold" width="100"
                        wire:click="sortByField('estado_status')" style="cursor: pointer;"
                        title="Clique para ordenar">
                        Status
                        @include('livewire.partials._sort-icon', ['field' => 'estado_status'])
                    </th>
                    <th class="px-2 py-2 font-semibold" wire:click="sortByField('estado_nome')"
                        style="cursor: pointer;" title="Clique para ordenar">
                        Nome do Estado
                        @include('livewire.partials._sort-icon', ['field' => 'estado_nome'])
                    </th>
                    <th class="px-2 py-2 font-semibold" wire:click="sortByField('estado_uf')"
                        style="cursor: pointer;" title="Clique para ordenar">
                        UF
                        @include('livewire.partials._sort-icon', ['field' => 'estado_uf'])
                    </th>
            ...
            <tbody>
                ...
                @forelse ($estados as $estado)
                    <tr class="border-t hover:bg-gray-50 transition" wire:loading.remove wire:target="searchTerm">
                        ...
                        <td class="px-2 py-1 text-sm font-medium text-gray-800">{{ $estado->estado_nome }}
                        <td class="px-2 py-1 text-sm font-medium text-gray-800">{{ $estado->estado_uf }}
                        </td>
                        <td class="px-2 py-1 text-center space-x-0">
                            <!-- Exibir -->
                            @can('Exibir Estado')
                            <a href="{{ route('admin.estados.detalhes', $estado->estado_id) }}"
                            ...
                            <!-- Editar -->
                            @can('Editar Estado')
                            <a href="{{ route('admin.estados.formulario', $estado->estado_id) }}"
                            ...
                            <!-- Deletar -->
                            @can('Deletar Estado')
                            <button type="button" wire:click="confirmDelete('{{ $estado->estado_id }}')"
                            ...
                            <div class="flex flex-col items-center justify-center space-y-1.5">
                                ...
                                @if (!$searchTerm)
                                    <a href="{{ route('admin.estados.formulario') }}"
                                        ...
                                        <i class="fa fa-plus"></i> Criar Primeiro Estado
                                    </a>
                                @endif
        ...
        <div class="block md:hidden divide-y divide-gray-100">
            @forelse ($estados as $estado)
                <div class="p-4 hover:bg-gray-50 transition">
                    <p class="font-semibold text-gray-800 mb-1">{{ $estado->estado_nome }}</p>
                    <p class="text-sm text-gray-600">
                        <strong>Status:</strong> {{ $estado->estado_status ?? '—' }}<br>
                    </p>

                    <!-- Botões -->
                    <div class="flex justify-end mt-2 gap-1">
                        <a href="/estados/{{ $estado->estado_id }}"
            ...
            @empty
                <p class="text-center text-gray-500 py-4">Nenhum estado encontrado.</p>
            @endforelse
        </div>
    </div>
    ...
    <div class="flex flex-col md:flex-row justify-between items-center gap-3">
        ...
        @if ($estados->total() > 0)
            <div class="text-sm text-gray-600">
                Mostrando
                <span class="font-semibold text-gray-800">{{ $estados->firstItem() }}</span>
                até
                <span class="font-semibold text-gray-800">{{ $estados->lastItem() }}</span>
                de
                <span class="font-semibold text-gray-800">{{ $estados->total() }}</span>
                registro(s)
                @if ($searchTerm)
    ...
    @if ($confirmingDelete)
        <div class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 px-4"
            ...
            <div class="bg-white rounded-lg p-4 w-full max-w-md shadow-lg">
                ...
                <p class="mb-4 text-gray-700">Tem certeza de que deseja excluir este estado?</p>
