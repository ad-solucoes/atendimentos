<div class="space-y-6">
            ...
            <div class="flex flex-col md:items-end">
                ...
                <div class="flex items-center justify-between md:justify-end gap-3">
                    ...
                    <small class="text-gray-600 text-sm">
                        ...
                        Total: <span class="font-semibold text-gray-800">{{ $municipios->total() }}</span>
                        registro(s)
                    </small>

                    @can('Adicionar Municipio')
                        <a href="{{ route('admin.municipios.formulario') }}"
            ...
            <thead>
                <tr class="bg-gray-100 text-left text-sm text-gray-600">
                    <th class="px-2 py-2 text-center font-semibold" width="100"
                        wire:click="sortByField('municipio_status')" style="cursor: pointer;"
                        title="Clique para ordenar">
                        Status
                        @include('livewire.partials._sort-icon', ['field' => 'municipio_status'])
                    </th>
                    <th class="px-2 py-2 font-semibold" wire:click="sortByField('municipio_nome')"
                        style="cursor: pointer;" title="Clique para ordenar">
                        Nome do Municipio
                        @include('livewire.partials._sort-icon', ['field' => 'municipio_nome'])
                    </th>
            ...
            <tbody>
                ...
                @forelse ($municipios as $municipio)
                    <tr class="border-t hover:bg-gray-50 transition" wire:loading.remove wire:target="searchTerm">
                        ...
                        <td class="px-2 py-1 text-sm font-medium text-gray-800">{{ $municipio->municipio_nome }}
                        </td>
                        <td class="px-2 py-1 text-sm font-medium text-gray-800">
                            {{ $municipio->estado->estado_nome }}</td>
                        <td class="px-2 py-1 text-center space-x-0">
                            <!-- Exibir -->
                            @can('Exibir Municipio')
                            <a href="{{ route('admin.municipios.detalhes', $municipio->municipio_id) }}"
                                class="w-7 h-6 inline-flex items-center justify-center bg-blue-100 text-blue-700 hover:bg-blue-200 p-2 transition"
                                title="Exibir Municipio">
                            ...
                            <a href="{{ route('admin.municipios.formulario', $municipio->municipio_id) }}"
                            ...
                            <!-- Deletar -->
                            @can('Deletar Municipio')
                            <button type="button" wire:click="confirmDelete('{{ $municipio->municipio_id }}')"
                            ...
                            <div class="flex flex-col items-center justify-center space-y-1.5">
                                ...
                                <div>
                                    ...
                                    @else
                                        ...
                                        <p class="text-sm text-gray-500 mt-1">
                                            Comece criando um novo municipio.
                                        </p>
                                    @endif
                                </div>
                                ...
                                @if (!$searchTerm)
                                    <a href="{{ route('admin.municipios.formulario') }}"
                                        ...
                                        <i class="fa fa-plus"></i> Criar Primeiro Municipio
                                    </a>
                                @endif
        ...
        <div class="block md:hidden divide-y divide-gray-100">
            @forelse ($municipios as $municipio)
                <div class="p-4 hover:bg-gray-50 transition">
                    <p class="font-semibold text-gray-800 mb-1">{{ $municipio->municipio_nome }}</p>
                    <p class="text-sm text-gray-600">
                        <strong>Status:</strong> {{ $municipio->municipio_status ?? '—' }}<br>
                    </p>

                    <!-- Botões -->
                    <div class="flex justify-end mt-2 gap-1">
                        <a href="/municipios/{{ $municipio->municipio_id }}"
            ...
            @empty
                <p class="text-center text-gray-500 py-4">Nenhum municipio encontrado.</p>
            @endforelse
        </div>
    </div>
    ...
    <div class="flex flex-col md:flex-row justify-between items-center gap-3">
        ...
        @if ($municipios->total() > 0)
            <div class="text-sm text-gray-600">
                Mostrando
                <span class="font-semibold text-gray-800">{{ $municipios->firstItem() }}</span>
                até
                <span class="font-semibold text-gray-800">{{ $municipios->lastItem() }}</span>
                de
                <span class="font-semibold text-gray-800">{{ $municipios->total() }}</span>
                registro(s)
                @if ($searchTerm)
    ...
    @if ($confirmingDelete)
        <div class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 px-4"
            ...
            <div class="bg-white rounded-lg p-4 w-full max-w-md shadow-lg">
                ...
                <p class="mb-4 text-gray-700">Tem certeza de que deseja excluir este municipio?</p>
