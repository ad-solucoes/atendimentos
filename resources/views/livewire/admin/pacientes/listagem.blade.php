    ...
    <div class="bg-white border border-gray-200 rounded-xl p-4 shadow-sm mb-4">
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between mt-4 gap-3">
            <div class="flex items-center gap-2">
                ...
                <button type="button"
                    wire:click="$set('filtroNome', ''); $set('filtroCpf', ''); $set('filtroSus', ''); $set('filtroMae', '')"
    ...
    <div class="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden">
        ...
        <table class="hidden md:table min-w-full">
            <thead>
                <tr class="bg-gray-100 text-left text-sm text-gray-600">
                    <th class="px-2 py-2 text-center font-semibold" width="100"
                        wire:click="sortByField('paciente_status')" style="cursor: pointer;"
                        title="Clique para ordenar">
                        Status
                        @include('livewire.partials._sort-icon', ['field' => 'paciente_status'])
                    </th>
                    <th class="px-2 py-2 font-semibold" wire:click="sortByField('paciente_nome')"
            ...
            <tbody>
                ...
                <tr wire:loading.class.remove="hidden" wire:target="filtroNome, filtroCpf, filtroSus, filtroMae"
                    class="hidden">
                ...
                @forelse ($pacientes as $paciente)
                    <tr class="border-t hover:bg-gray-50 transition" wire:loading.remove
                        wire:target="filtroNome, filtroCpf, filtroSus, filtroMae">
                        ...
                        <td class="px-2 py-1 text-center space-x-0">
                            ...
                            <!-- Deletar -->
                            @can('Deletar Paciente')
                            <button type="button" wire:click="confirmDelete('{{ $paciente->paciente_id }}')"
