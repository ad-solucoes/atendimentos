            ...
            <div class="flex flex-col md:items-end">
                ...
                <div class="flex items-center justify-between md:justify-end gap-3">
                    ...
                    <small class="text-gray-600 text-sm">
                        ...
                        Total: <span class="font-semibold text-gray-800">{{ $perfis->total() }}</span> registro(s)
                    </small>

                    @can('Adicionar Perfil')
                        <a href="{{ route('admin.perfis.formulario') }}"
            ...
            <tbody>
                ...
                @forelse ($perfis as $perfil)
                    <tr class="border-t hover:bg-gray-50 transition" wire:loading.remove wire:target="searchTerm">
                        <td class="px-2 py-1 text-center text-sm font-medium text-gray-800">{{ $perfil->id }}</td>
                        <td class="px-2 py-1 text-sm font-medium text-gray-800">{{ $perfil->name }}</td>
                        <td class="px-2 py-1 text-center space-x-0">
                            <!-- Gerenciar -->
                            @can('Gerenciar Permissao')
                            <a href="{{ route('admin.perfis.permissoes', $perfil->id) }}"
                            ...
                            <!-- Exibir -->
                            @can('Exibir Perfil')
                            <a href="{{ route('admin.perfis.detalhes', $perfil->id) }}"
                            ...
                            <!-- Editar -->
                            @can('Editar Perfil')
                            <a href="{{ route('admin.perfis.formulario', $perfil->id) }}"
                            ...
                            <!-- Deletar -->
                            @can('Deletar Perfil')
                            <button type="button" wire:click="confirmDelete('{{ $perfil->id }}')"
                            ...
                            <div class="flex flex-col items-center justify-center space-y-1.5">
                                ...
                                @if (!$searchTerm)
                                    <a href="{{ route('admin.perfis.formulario') }}"
                                        ...
                                        <i class="fa fa-plus"></i> Criar Primeiro Perfil
                                    </a>
                                @endif
        ...
        <div class="block md:hidden divide-y divide-gray-100">
            @forelse ($perfis as $perfil)
                <div class="p-4 hover:bg-gray-50 transition">
                    <p class="font-semibold text-gray-800 mb-1">{{ $perfil->name }}</p>
                    <p class="text-sm text-gray-600">
                        <strong>Status:</strong> {{ $perfil->perfil_status ?? '—' }}<br>
                    </p>

                    <!-- Botões -->
                    <div class="flex justify-end mt-2 gap-1">
                        <a href="/perfis/{{ $perfil->id }}"
            ...
            @empty
                <p class="text-center text-gray-500 py-4">Nenhum perfil encontrado.</p>
            @endforelse
        </div>
    </div>
    ...
    <div class="flex flex-col md:flex-row justify-between items-center gap-3">
        ...
        @if ($perfis->total() > 0)
            <div class="text-sm text-gray-600">
                Mostrando
                <span class="font-semibold text-gray-800">{{ $perfis->firstItem() }}</span>
                até
                <span class="font-semibold text-gray-800">{{ $perfis->lastItem() }}</span>
                de
                <span class="font-semibold text-gray-800">{{ $perfis->total() }}</span>
                registro(s)
                @if ($searchTerm)
    ...
    @if ($confirmingDelete)
        <div class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 px-4"
            ...
            <div class="bg-white rounded-lg p-4 w-full max-w-md shadow-lg">
                ...
                <p class="mb-4 text-gray-700">Tem certeza de que deseja excluir este perfil?</p>
