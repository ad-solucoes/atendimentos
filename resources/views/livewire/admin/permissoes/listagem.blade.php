            ...
            <div class="flex flex-col md:items-end">
                ...
                <div class="flex items-center justify-between md:justify-end gap-3">
                    ...
                    <small class="text-gray-600 text-sm">
                        ...
                        Total: <span class="font-semibold text-gray-800">{{ $permissoes->total() }}</span> registro(s)
                    </small>

                    @can('Adicionar Permissao')
                        <a href="{{ route('admin.permissoes.formulario') }}"
        ...
        <table class="hidden md:table min-w-full">
            <thead>
                <tr class="bg-gray-100 text-left text-sm text-gray-600">
                    <th class="px-2 py-2 text-center font-semibold" width="80" wire:click="sortByField('id')"
                        style="cursor: pointer;" title="Clique para ordenar">
                        ID
                        @include('livewire.partials._sort-icon', ['field' => 'id'])
                    </th>
            ...
            <tbody>
                ...
                @forelse ($permissoes as $permissao)
                    <tr class="border-t hover:bg-gray-50 transition" wire:loading.remove wire:target="searchTerm">
                        <td class="px-2 py-1 text-center text-sm font-medium text-gray-800">{{ $permissao->id }}</td>
                        <td class="px-2 py-1 text-sm font-medium text-gray-800">{{ $permissao->name }}</td>
                        <td class="px-2 py-1 text-center space-x-0">
                            <!-- Exibir -->
                            @can('Exibir Permissao')
                            <a href="{{ route('admin.permissoes.detalhes', $permissao->id) }}"
                                class="w-7 h-6 inline-flex items-center justify-center bg-blue-100 text-blue-700 hover:bg-blue-200 p-2 transition"
                                title="Exibir Permissão">
                            ...
                            <a href="{{ route('admin.permissoes.formulario', $permissao->id) }}"
                            ...
                            <button type="button" wire:click="confirmDelete('{{ $permissao->id }}')"
                            ...
                            <div class="flex flex-col items-center justify-center space-y-1.5">
                                ...
                                @if (!$searchTerm)
                                    <a href="{{ route('admin.permissoes.formulario') }}"
                                        ...
                                        <i class="fa fa-plus"></i> Criar Primeiro Permissão
                                    </a>
                                @endif
        ...
        <div class="block md:hidden divide-y divide-gray-100">
            @forelse ($permissoes as $permissao)
                <div class="p-4 hover:bg-gray-50 transition">
                    <p class="font-semibold text-gray-800 mb-1">{{ $permissao->name }}</p>
                    <p class="text-sm text-gray-600">
                        <strong>Status:</strong> {{ $permissao->permissao_status ?? '—' }}<br>
                    </p>

                    <!-- Botões -->
                    <div class="flex justify-end mt-2 gap-1">
                        <a href="/permissoes/{{ $permissao->id }}"
            ...
            @empty
                <p class="text-center text-gray-500 py-4">Nenhum permissao encontrado.</p>
            @endforelse
        </div>
    </div>
    ...
    <div class="flex flex-col md:flex-row justify-between items-center gap-3">
        ...
        @if ($permissoes->total() > 0)
            <div class="text-sm text-gray-600">
                Mostrando
                <span class="font-semibold text-gray-800">{{ $permissoes->firstItem() }}</span>
                até
                <span class="font-semibold text-gray-800">{{ $permissoes->lastItem() }}</span>
                de
                <span class="font-semibold text-gray-800">{{ $permissoes->total() }}</span>
                registro(s)
                @if ($searchTerm)
    ...
    @if ($confirmingDelete)
        <div class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 px-4"
            ...
            <div class="bg-white rounded-lg p-4 w-full max-w-md shadow-lg">
                ...
                <p class="mb-4 text-gray-700">Tem certeza de que deseja excluir este permissao?</p>
