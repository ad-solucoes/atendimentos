<div class="max-w-4xl mx-auto px-0 sm:px-6 lg:px-8 py-">
    ...
    <form wire:submit.prevent="save" class="space-y-4">
        ...
        <div>
            <label class="block font-semibold mb-1">Nome do Procedimento *</label>
            ...
            @error('procedimento_nome') <span class="text-red-600 text-sm">{{ $message }}</span> @enderror
        </div>

        <!-- Tipo de Procedimento -->
        <div>
            <label class="block font-semibold mb-1">Tipo de Procedimento</label>
            ...
            @error('procedimento_tipo_id') <span class="text-red-600 text-sm">{{ $message }}</span> @enderror
        </div>

        <!-- Botões -->
        <div class="pt-4 flex flex-col sm:flex-row justify-center sm:space-x-2 space-y-2 sm:space-y-0">
            <a href="{{ route('admin.procedimentos.listagem') }}" class="px-4 py-2 border rounded w-full sm:w-auto text-center">Cancelar</a>
