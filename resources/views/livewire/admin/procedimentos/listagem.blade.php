        ...
        <table class="hidden md:table min-w-full">
            <thead>
                <tr class="bg-gray-100 text-left text-sm uppercase text-gray-600">
                    <th class="px-4 py-3">Nome do Procedimento</th>
            ...
            <tbody>
                @forelse ($procedimentos as $procedimento)
                    <tr class="border-t hover:bg-gray-50 transition">
                        <td class="px-4 py-2 font-medium text-gray-800">{{ $procedimento->procedimento_nome }}</td>
                        <td class="px-4 py-2 font-medium text-gray-800">{{ $procedimento->tipo_procedimento->tipo_procedimento_nome }}</td>
                ...
                @empty
                    <tr><td colspan="5" class="text-center py-4 text-gray-500">Nenhum procedimento encontrado.</td></tr>
                @endforelse
            </tbody>
        </table>
        ...
        <div class="block md:hidden divide-y divide-gray-100">
            @forelse ($procedimentos as $procedimento)
                <div class="p-4 hover:bg-gray-50 transition">
                    <p class="font-semibold text-gray-800 mb-1">{{ $procedimento->procedimento_nome }}</p>
                    <p class="text-sm text-gray-600">
                        <strong>Status:</strong> {{ $procedimento->procedimento_status ?? '—' }}<br>
                    </p>
            ...
            @empty
                <p class="text-center text-gray-500 py-4">Nenhum procedimento encontrado.</p>
            @endforelse
        </div>
    </div>

    <!-- Paginação -->
    <div class="mt-4">{{ $procedimentos->links() }}</div>
