            ...
            <div class="flex flex-col md:items-end">
                ...
                <div class="flex items-center justify-between md:justify-end gap-3">
                    ...
                    <small class="text-gray-600 text-sm">
                        ...
                        Total: <span class="font-semibold text-gray-800">{{ $setores->total() }}</span> registro(s)
                    </small>

                    <a href="{{ route('admin.setores.formulario') }}"
        ...
        <table class="hidden md:table min-w-full">
            <thead>
                <tr class="bg-gray-100 text-left text-sm text-gray-600">
                    <th class="px-2 py-2 text-center font-semibold" width="100" wire:click="sortByField('setor_status')" style="cursor: pointer;" title="Clique para ordenar">
                        Status
                        @include('livewire.partials._sort-icon', ['field' => 'setor_status'])
                    </th>
                    <th class="px-2 py-2 font-semibold" wire:click="sortByField('setor_nome')" style="cursor: pointer;" title="Clique para ordenar">
                        Nome do Setor
                        @include('livewire.partials._sort-icon', ['field' => 'setor_nome'])
                    </th>
            ...
            <tbody>
                ...
                @forelse ($setores as $setor)
                    <tr class="border-t hover:bg-gray-50 transition" wire:loading.remove wire:target="searchTerm">
                        ...
                        <td class="px-2 py-1 text-sm font-medium text-gray-800">{{ $setor->setor_nome }}</td>
                        <td class="px-2 py-1 text-center space-x-0">
                            <!-- Ver -->
                            <a href="{{ route('admin.setores.detalhes', $setor->setor_id) }}"
                            ...
                            <!-- Editar -->
                            <a href="{{ route('admin.setores.formulario', $setor->setor_id) }}"
                            ...
                            <!-- Excluir -->
                            <button type="button"
                                    wire:click="confirmDelete('{{ $setor->setor_id }}')"
                    ...
                    <tr wire:loading.remove wire:target="searchTerm">
                        <td colspan="4" class="py-12 text-center">
                            <div class="flex flex-col items-center justify-center space-y-1.5">
                                ...
                                @if(!$searchTerm)
                                    <a href="{{ route('admin.setores.formulario') }}" class="mt-3 inline-flex items-center gap-2 bg-blue-700 hover:bg-blue-800 text-white text-sm font-medium px-4 py-2.5 rounded-lg shadow transition-all duration-200">
        ...
        <div class="block md:hidden divide-y divide-gray-100">
            @forelse ($setores as $setor)
                <div class="p-4 hover:bg-gray-50 transition">
                    <p class="font-semibold text-gray-800 mb-1">{{ $setor->setor_nome }}</p>
                    <p class="text-sm text-gray-600">
                        <strong>Status:</strong> {{ $setor->setor_status ?? '—' }}<br>
                    </p>

                    <!-- Botões -->
                    <div class="flex justify-end mt-2 gap-1">
                        <a href="/setores/{{ $setor->setor_id }}" 
            ...
            @empty
                <p class="text-center text-gray-500 py-4">Nenhum setor encontrado.</p>
            @endforelse
        </div>
    </div>
    ...
    <div class="flex flex-col md:flex-row justify-between items-center gap-3">
        ...
        @if ($setores->total() > 0)
            <div class="text-sm text-gray-600">
                Mostrando
                <span class="font-semibold text-gray-800">{{ $setores->firstItem() }}</span>
                até
                <span class="font-semibold text-gray-800">{{ $setores->lastItem() }}</span>
                de
                <span class="font-semibold text-gray-800">{{ $setores->total() }}</span>
                registro(s)
                @if ($searchTerm)
    ...
    @if($confirmingDelete)
        <div class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 px-4" style="margin-top: 0!important">
            <div class="bg-white rounded-lg p-4 w-full max-w-md shadow-lg">
                ...
                <p class="mb-4 text-gray-700">Tem certeza de que deseja excluir este setor?</p>
